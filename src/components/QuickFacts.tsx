...
export default function QuickFacts({
  ...
  return (
    <div
      className={`rounded-2xl border border-charcoal/10 bg-white p-6 sm:p-7 ${className}`}
    >
      <p className="text-sm font-semibold uppercase tracking-[0.14em] text-fresh-green">
        Auf einen Blick
      </p>
      <dl className="mt-4 flex flex-col divide-y divide-dashed divide-charcoal/12 text-sm">
        {rows.map((row) => (
          <div
            key={row.label}
            className="flex flex-col gap-1 py-2.5 first:pt-0 last:pb-0 sm:flex-row sm:gap-6"
          >
            <dt className="font-semibold text-charcoal sm:w-40 sm:shrink-0">
              {row.label}
            </dt>
            <dd className="text-charcoal/70">{row.value}</dd>
          </div>
        ))}
      </dl>
    </div>
